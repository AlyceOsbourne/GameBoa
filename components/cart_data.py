...
def get_header_data(rom: array.array):
    mapping = {
        k: v for k, v in zip(
    ...
    return HeaderData(**mapping)


def get_rom_banks(rom: array.array, header: HeaderData) -> list[Memory]:
    banks = []
    for i in range(getattr(header, "rom_size")// ROM_BANK_SIZE):
        banks.append(Memory(rom[ROM_BANK_START + i * ROM_BANK_SIZE: ROM_BANK_END + i * ROM_BANK_SIZE]))
    return banks



def get_ram_banks(header: HeaderData) -> list[Memory]:
    banks = []
    for bank in range(getattr(header, 'ram_size') // RAM_BANK_SIZE):
        banks.append(Memory(RAM_BANK_SIZE))

    return banks


def get_cart(path: Path) -> tuple:
    with open(path, 'rb') as f:
        rom = array.array('B', f.read())
    header = get_header_data(rom)
    rom_banks = get_rom_banks(rom, header)
    ram_banks = get_ram_banks(header)
    return header, rom_banks, ram_banks


def test():
    from pprint import pprint
    rom_path = Path(__file__).parent.parent / 'roms' / 'Tetris.gb'
    header, rom_banks, ram_banks = get_cart(rom_path)
    pprint(header._asdict())
    print(rom_banks[0].read_bytes(0x100, 32))
