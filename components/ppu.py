...
from protocols import MemoryBankProtocol, PPUProtocol
...
class PPU:
    ...
    obp1: int = 0
    stat: int = 0
    oam: MemoryBankProtocol = MemoryBank(len(PPUReadWriteRanges.OAM))
    vram: MemoryBankProtocol = MemoryBank(len(PPUReadWriteRanges.VRAM))
...
class ScreenConsoleRenderer:
    ...
    def update(self, ppu: PPUProtocol):
        """Updates the screen of the given ppu."""
        for y in range(self.screen_size.height):
            for x in range(self.screen_size.width):
