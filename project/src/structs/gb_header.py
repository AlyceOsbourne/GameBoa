...
RAM_SIZES = MappingProxyType(
    {0x00: 0x0000, 0x01: 0x0002, 0x02: 0x0008, 0x03: 0x0020, 0x04: 0x0080, 0x05: 0x0200}
)
...
def get_header_data(rom: array):
    mapping = {
        ...
        for k, v in zip(
            ...
            struct.unpack_from(
                "<" + "".join(f[1] for f in HEADER_FORMAT), rom[HEADER_START:HEADER_END]
            ),
        )
    }
    ...
    mapping["old_licensee_code"] = OLD_LICENSEE_CODES.get(
        mapping["old_licensee_code"], "Unknown"
    )
    mapping["new_licensee_code"] = NEW_LICENSEE_CODES.get(
        mapping["new_licensee_code"], "Unknown"
    )
